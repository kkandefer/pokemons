...
class PokemonsListCubit extends Cubit<PokemonsListState> {
  ...
  void initList() async {
    ...
    try{
      ...
      Future.forEach(miniTabs, (List<PokemonListItem> miniTab) async {
        await _fillImages(miniTab.where((i) => i.imageUrl == null).toList(), state.searchPhrase);
      });
    }
    catch(e, s) {
      emit(state.copyWith(
  ...
  void append({ int? limit }) async {
    ...
    late List<PokemonListItem> sortedResults;
    try{
      if(state.searchPhrase != null){
        sortedResults = await pokemonListItemDao.findPokemonsPart(state.searchPhrase!, state.results.length, limit ?? _limit);
      }
      else{
        sortedResults = await pokemonListItemDao.getPokemonsPart(state.results.length, limit ?? _limit);
      }
      emit(state.copyWith(
  ...
  void search(String phrase) async {
    ...
    try{
      List<PokemonListItem> sortedResults = await pokemonListItemDao.findPokemonsPart('%${phrase}%', 0, _limit);
      emit(state.copyWith(
        results: (sortedResults).unique((i) => i.id),
        searchStatus: PokemonsListStatus.normal,
        hasRatherMax: sortedResults.isEmpty,
      ));
      ...
      log.severe('Unable to search in database', e, s);
      //TODO logowanie do crashlytics, ale w tej wersji nie podłaczam bo nie ma konta
    }
  }
  ...
  Future<void> _fillImages(List<PokemonListItem> itemsToFill, String? startSearchPhrase) async {
    // startSearchPhrase to bezpiecznik dla zmiany warunków wyszukiwania
    // ale szczerze mówiąc cały ten flow jest do zaorania, tylko już nie mam na to czas :/
    Map<int, String?> mapOfImages = {};
    await Future.forEach(itemsToFill, (PokemonListItem item) async {
      // print('_fillImages $startSearchPhrase - ${state.searchPhrase}');
      if(item.imageUrl == null && startSearchPhrase == state.searchPhrase){
        String? image = await pokemonsRepository.getRemotePokemonImage(id: item.id);
        mapOfImages.addAll({item.id: image});
      }
    });
    if(mapOfImages.isNotEmpty){
